...
#define	MAP_SIZE 1608            // Размер карты высот Terrain.RAW
#define	STEP_SIZE 1            // Шаг выставления точек высоты
int STEP_GRID = 804;  // Шаг выставления точек высоты
//----------------------------------------------------------------------------
#pragma argsused
//----------------------------------------------------------------------------
HGLRC hRC = NULL;
...
//----------------------------------------------------------------------------
BYTE g_HeightMap[ MAP_SIZE*MAP_SIZE ];	// Карта массива с высотами

GLfloat xrot = -50.4;	        // X начальный угол поворота
GLfloat yrot = 0.3;		        // Y
GLfloat zrot = 60.4;	        	  // Z
GLuint texture[ 1 ];      	     // Массив с картой месности(текстурой)
//float points[ STEP_GRID ][ STEP_GRID][ 3 ];  // Массив узловых точек полигона
float points[ MAP_SIZE ][ MAP_SIZE][ 3 ];  // Массив узловых точек полигона

float xa=0, ya=0, za=0;
float ScaleMap = -37;   // Масштаб карты
float ScaleHeight = 0.335; // Масштаб вершин
...
int InitGL(GLvoid)      // Инициализация параметров OpenGL
{
   ...
   glPolygonMode(GL_BACK, GL_FILL);	   // Прорисовка изнанки
	glPolygonMode(GL_FRONT, GL_LINE);	// Прорисовка верха

	for(int x = 0; x < STEP_GRID; x++)
		for(int y = 0; y < STEP_GRID; y++)
		{
			points[x][y][0] = float((x/5.0f)-4.5f); //центрируем
			points[x][y][1] = float((y/5.0f)-4.5f);
			points[x][y][2] = 1;                    // Начальный уровень высоты
		}
   ...
   int x, y, z;
	...
	for( x = 0; x <(STEP_GRID-1); x++ )
		for( y = 0; y <(STEP_GRID-1); y++ )
		{
         ...
         glTexCoord2f(float_x, float_y);	// левый нижний
			glVertex3f( points[x][y][0], points[x][y][1], points[x][y][2]);

			glTexCoord2f( float_x, float_yb );	// левый верхний
			glVertex3f(points[x][y+1][0], points[x][y+1][1], points[x][y+1][2]);

			glTexCoord2f( float_xb, float_yb );	// правый верхний
			glVertex3f( points[x+1][y+1][0], points[x+1][y+1][1], points[x+1][y+1][2]);

			glTexCoord2f( float_xb, float_y );	// правый нижний
			glVertex3f( points[x+1][y][0], points[x+1][y][1], points[x+1][y][2]);
		}
	glEnd();

	int X = 0, Y = 0;
   glBegin( GL_QUADS );		// Рендеринг полигона
	glBegin( GL_LINES );		// Рендеринг линий
  	for ( X = 0; X < (STEP_GRID); X = X + STEP_SIZE)
		for ( Y = 0; Y < (STEP_GRID); Y = Y + STEP_SIZE)
		{
         ...
         points[X][Y][2] = float(z * ScaleHeight);   
			...
			//glVertex3i(float(x/5.0f)-wMov, float(y/5.0f)-hMov, float(z * ScaleHeight));	// Рендеринг вершин(можно отключить, но так быстрее)
